...
router.post('/gather', async (req: Request, res: Response) => {
  try {
    const request: EnhancedContextRequest = {
      request_id: req.body.request_id || `ctx_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      template_id: req.body.template_id,
      project_id: req.body.project_id,
      user_id: req.body.user_id || req.user?.id,
      document_type: req.body.document_type || 'general',
      gathering_config: req.body.gathering_config || {
        context_sources: [
          {
        ...
        enable_external_source_integration: req.body.enable_external_sources || false,
        enable_rag_integration: req.body.enable_rag || true,
        enable_baseline_integration: req.body.enable_baseline || true
      },
      enable_access_control: req.body.enable_access_control ?? true,
      enable_freshness_validation: req.body.enable_freshness_validation ?? true,
      freshness_threshold: req.body.freshness_threshold || 86400000, // 24 hours
      required_permissions: req.body.required_permissions || ['read'],
      context_size_limit: req.body.context_size_limit || 10 * 1024 * 1024 // 10MB
    }
