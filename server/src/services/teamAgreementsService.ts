  ...
  private mapRowToAgreement(row: Record<string, unknown>, userMap?: Record<string, UserSummary>): TeamAgreement {
    try {
      ...
      return {
        ...
        facilitated_by_name: row.facilitated_by_name ? String(row.facilitated_by_name) : (facilitatedById && userMap?.[facilitatedById]?.name) ?? null,
        ...
        created_by_name: row.created_by_name ? String(row.created_by_name) : (createdById && userMap?.[createdById]?.name) ?? null
      }
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
