...
CREATE TABLE IF NOT EXISTS ibabs_connections (
  ...
  access_token TEXT NOT NULL, -- Encrypted using base64 encoding
  refresh_token TEXT NOT NULL, -- Encrypted using base64 encoding
...
COMMENT ON TABLE ibabs_connections IS 'OAuth 2.0 token storage for iBabs board portal integration. Stores encrypted access and refresh tokens with automatic expiration tracking.';
...
COMMENT ON COLUMN ibabs_connections.access_token IS 'OAuth access token (base64 encoded for storage)';
COMMENT ON COLUMN ibabs_connections.refresh_token IS 'OAuth refresh token (base64 encoded for storage)';
