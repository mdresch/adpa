...
-- NOTE: entity_id+version is not actually unique in practice -- concurrent
-- writes can assign the same version number to two distinct audit events
-- (a pre-existing race condition in whatever assigns `version`), and an
-- audit trail must never drop a legitimate record to satisfy a constraint.
-- Non-unique by design; do not change back to CREATE UNIQUE INDEX.
CREATE INDEX IF NOT EXISTS idx_entity_audit_entity_id ON public.entity_audit_trail(entity_id);
CREATE INDEX IF NOT EXISTS idx_entity_audit_entity_version ON public.entity_audit_trail(entity_id, version);
