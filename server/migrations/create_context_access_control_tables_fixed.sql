-- Context Access Control Database Schema (Fixed)
...
-- User Roles Table (Fixed to use UUID for user_id)
CREATE TABLE IF NOT EXISTS user_roles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    role_id VARCHAR(255) NOT NULL REFERENCES roles(id) ON DELETE CASCADE,
    context_id UUID, -- Will reference contexts table when it exists
...
-- User Permissions Table (Fixed to use UUID for user_id)
...
-- Contexts Table (Simplified without foreign key constraints for now)
CREATE TABLE IF NOT EXISTS contexts (
    ...
    owner_id UUID NOT NULL, -- Will reference users(id) when needed
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Access Attempts Table (Fixed to use UUID for user_id)
CREATE TABLE IF NOT EXISTS access_attempts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    attempt_id VARCHAR(255) UNIQUE NOT NULL,
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    context_id UUID NOT NULL, -- Will reference contexts(id) when contexts table is properly set up
    action VARCHAR(50) NOT NULL CHECK (action IN ('read', 'write', 'update', 'delete', 'share', 'export', 'import', 'admin', 'audit')),
...
-- Access Logs Table (Fixed to use UUID for user_id)
...
CREATE TABLE IF NOT EXISTS security_policies (
    ...
    context_id UUID, -- Will reference contexts(id) when contexts table is properly set up
    rules JSONB DEFAULT '[]',
...
-- Access Grant Log Table (Fixed to use UUID for user_id)
...
-- Access Revocation Log Table (Fixed to use UUID for user_id)
...
-- Access Update Log Table (Fixed to use UUID for user_id)
...
-- Role Assignment Log Table (Fixed to use UUID for user_id)
...
-- Role Removal Log Table (Fixed to use UUID for user_id)
...
BEGIN
    ...
    INSERT INTO access_update_log (
        ...
        '00000000-0000-0000-0000-000000000000'::uuid, '00000000-0000-0000-0000-000000000000'::uuid, '[]', 'system', NOW(), 'Cleanup expired access control'
    );
END;
$$ LANGUAGE plpgsql;
