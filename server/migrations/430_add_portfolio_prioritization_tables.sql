-- SC-78: Portfolio prioritization tables (portfolio_criteria, portfolio_scores,
-- portfolio_rankings materialized view).
--
-- Ported from the separate Supabase-CLI-tracked
-- supabase/migrations/20260507105500_portfolio_prioritization_tables.sql,
-- which run-migrations.ts never applies (it only reads server/migrations/) --
-- so this table was never actually created through the pipeline. Content
-- is unchanged from the source file; its RLS policies already use
-- USING (true) / plain EXISTS subqueries (no auth.*), so no Azure-specific
-- rewrite was needed here (unlike migration 428's strategic_goals port).
