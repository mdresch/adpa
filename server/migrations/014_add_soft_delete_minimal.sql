-- Migration: Add soft delete functionality to documents table (minimal)
...
-- Create a view for active (non-deleted) documents with existing columns only
CREATE OR REPLACE VIEW documents_active AS
SELECT 
    d.*,
    CASE 
        WHEN d.status = 'generated' THEN 'Generated'
    ...
    END as status_label,
    CASE 
        WHEN d.template_id IS NOT NULL THEN 'ai_generated'
        WHEN d.file_path IS NOT NULL THEN 'user_upload'
        ELSE 'manual'
    END as source_type,
    ...
    CASE 
        WHEN d.status = 'generated' AND EXTRACT(EPOCH FROM (NOW() - d.created_at)) / 3600 > 24 THEN 'overdue'
        WHEN d.status = 'generated' AND EXTRACT(EPOCH FROM (NOW() - d.created_at)) / 3600 > 12 THEN 'warning'
        ELSE 'ok'
    END as sla_status
FROM documents d
...
-- Create a view for deleted documents with existing columns only
...
SELECT 
    ...
    EXTRACT(EPOCH FROM (d.deleted_at - d.created_at)) / 3600 as lifespan_hours
FROM documents d
LEFT JOIN users u ON d.created_by = u.id
