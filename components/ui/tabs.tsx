...
const TabsTrigger = React.forwardRef(function TabsTrigger(
  { className, style, ...props }: React.ComponentPropsWithoutRef<typeof TabsPrimitive.Trigger>,
  ref: React.ForwardedRef<React.ElementRef<typeof TabsPrimitive.Trigger>>
) {
  // Get active state styles from inline style if provided
  const activeBg = style && (style as any)['--active-bg'] ? (style as any)['--active-bg'] : undefined;
  const activeText = style && (style as any)['--active-text'] ? (style as any)['--active-text'] : undefined;
