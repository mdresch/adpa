...
**Scope correction (caught during implementation)**: the plan originally listed `Capabilities.razor` alongside `Approvals.razor`/`MyRequests.razor` for this marker. Verified against the schema: `capability_registry` (what `Capabilities.razor` lists) never appears as an `audit_log.table_name` anywhere in this codebase — `promote_capability_status` writes only to `capability_activation_history`, never `audit_log`. A `capability_registry` row therefore has no `audit_log` entry of its own to point to; only `capability_override_requests`/`capability_override_exceptions` rows do (via PR3's `insertAuditLogDigest` and PR6a's `decide_capability_request`). This PR ships the marker on `Approvals.razor` and `MyRequests.razor` only, where a real chain entry always exists. `Capabilities.razor`'s own marker is deferred: giving it one would mean joining `capability_activation_history` into `CapabilityRegistryRepository.listForUser` and extending its response DTO — the same method and DTO already being extended by the separate, concurrently in-progress ADR-013 (attestation deadline / drift-reason visibility), which touches this exact query for an unrelated reason. Bundling `Capabilities.razor`'s provenance marker into that follow-up (or a PR right after it) avoids two independent edits landing on the same method at the same time; it is not dropped, only resequenced.

**Tasks**:
1. A small provenance marker on every row in `Approvals.razor` and `MyRequests.razor`, using **one canonical format across both pages** (ADR-012 itself deliberately left the exact display open — "hash-chain position or recorded timestamp" — but two pages independently choosing among that ambiguity would produce two different UIs for the same concept):
   ```
   Chain Entry: <audit_log.id>
   Recorded: <audit_log.occurred_at, UTC>
   ```
   The UI never displays the digest/hash itself — that's an integrity proof for reconciliation tooling, not something a reviewer reads directly. The `audit_log.id` + `occurred_at` pair is what gives a reviewer a stable reference they can correlate against reconciliation output or an audit export, while staying compact enough for routine display. This is the UI-side counterpart to ADR-005 Item 6's reconciliation job, now meaningful because PR3/PR6c guarantee every row actually has a chain entry to point to. For an override request, the marker reflects the **latest** `audit_log` row for that `capability_override_requests.id` (the create-time entry while `pending`, the `decide_capability_request`-written entry once approved/denied/withdrawn) — the most recent entry is the one that proves the row's *current* state is chained, not just its creation. For an exception, the marker reflects its own create-time `audit_log` row (`table_name = 'capability_override_exceptions'`) — `decideReview`'s audit entries are written against `override_exception_reviews`, one per reviewer decision, and are not rolled up into the top-level exception marker by this PR (a per-review marker is a natural future extension, not required by ADR-012 §8's literal text, which speaks to "every row" of the list, not its nested review sub-rows).
