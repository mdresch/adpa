...
## 1. Status
**Proposed (2026-07-15) — revised 2026-07-17** after a code-level review: §D corrected against the actual state of `capability_override_requests` (no existing DB-layer lockdown, no existing decision procedure), withdraw scoped to override requests only, ledger coverage re-sequenced ahead of the UI forms, and Governed Feature Loop obligations made explicit. **Revised again 2026-07-17 (PR2 implementation)**: §A and §6 Item 2 corrected — `exceptions/pending`'s authorization was misdescribed as sharing the override request's department-membership predicate; it's actually a distinct named-reviewer-assignment mechanism (verified against `CapabilityOverrideExceptionRepository.listPendingForUser`) and is not a consumer of Action Item 2's shared helper.
...
- **Node**: add `GET /api/v1/capability-registry`, authenticated, scoped the same way `overrides/pending` already is (admin/super_admin sees every capability; a plain user sees only capabilities whose `functional_owner_department` they're an active member of, in that portfolio) — consuming the shared admin-sees-all/member-sees-own-department helper extracted beforehand (Action Item 2), not a third copy of that logic. **Correction (2026-07-17, verified against the actual repositories):** an earlier draft of this ADR asserted that `exceptions/pending` shares this same department-membership predicate. It does not — `CapabilityOverrideExceptionRepository.listPendingForUser`'s non-admin branch authorizes by *named-reviewer-assignment* (`EXISTS (... override_exception_reviews rev WHERE rev.reviewer_user_id = $2 AND rev.decision IS NULL)`), a different table and a different mechanism entirely, unrelated to department membership — matching its own doc comment ("a plain user sees only exceptions where THEY are a named reviewer... matches `decideReview`'s own authorization"). Only `CapabilityOverrideRequestRepository.listPendingForUser` genuinely shares this list endpoint's predicate. The subtlety item 2 must resolve rather than paper over: `CapabilityOverrideRequestRepository.listPendingForUser` filters on the *request's* `requested_by_department`, while this list must filter on the *capability's* `functional_owner_department`. These usually carry the same value — the request column is populated from the capability's at creation time — but they are not the same predicate, and a capability with no pending requests has no request row to scope by at all. The shared helper names `functional_owner_department` (joined per portfolio) as the canonical scoping column; otherwise "one shared helper" quietly becomes two. `exceptions/pending` is not a consumer of this helper and Action Item 2 does not touch it — its reviewer-assignment authorization is already correct and is a different concern.
...
`withdraw` additionally needs the write-lockdown discipline ADR-005 Phase 3 task 2 applies to `activation_status` — but stating that honestly requires correcting a premise: `capability_override_requests.status` is *not* currently DB-enforced anywhere. `markApproved`/`markDenied` are plain app-layer `UPDATE`s, and no request-decision stored procedure exists to extend. The decision: lock down the whole `status` column (and its accompanying decision columns) at once — one `decide_capability_request` stored procedure becomes the only permitted writer for all three transitions (`approved`, `denied`, `withdrawn`), and `markApproved`/`markDenied` are migrated to call the procedure. That is more scope than withdraw alone, and this ADR owns it (Action Item 6) rather than leaving the newest, least-dangerous transition as the only one held to ADR-005's standard. The procedure also performs the audit_log insert itself — same transaction by construction, enforced at the DB layer, and no transaction-client plumbing through repositories that currently take a bare pool.

**Correction (2026-07-17, PR6a implementation): the lockdown mechanism is a trigger + session-local guard variable, not `REVOKE`.** An earlier draft of this section said "`UPDATE` is revoked from the application role" — verified against `adpa-capability-activation-lifecycle`'s own documented invariant for `activation_status`'s existing lockdown (the exact precedent this section cites) and it is explicitly **not** `REVOKE`-based: "`000_baseline.sql` and every migration were grepped for `REVOKE`: zero hits. This app also connects to Postgres via one `DATABASE_URL`/role for both migrations and runtime — there's no separate low-privilege app role to revoke from without rewiring deployment credentials." `activation_status`'s real mechanism is a `BEFORE UPDATE` trigger (`guard_capability_registry_activation_status`) checking a session-local guard (`current_setting('adpa.allow_activation_status_write', true)`) that only `promote_capability_status` sets, then resets immediately after its own write — enforced even against a superuser session, no role separation required. `capability_override_requests`'s lockdown mirrors this exactly: a `BEFORE UPDATE` trigger guards `status`/`approved_by`/`approved_by_department`/`decided_at`/`denial_reason`/`withdrawn_at`/`override_expires_at` together, `decide_capability_request` sets/resets the guard around its own `UPDATE`, and no deployment-credential rewiring is needed. Rewiring deployment credentials to introduce a role this codebase doesn't otherwise have would have been a materially bigger, riskier change than this ADR's stated scope.
...
2. Extract the admin-sees-all/member-sees-own-department scoping logic — genuinely present only in `overrides/pending` (`exceptions/pending` uses an unrelated named-reviewer-assignment predicate, not department membership; see §A's corrected predicate note) — into one shared repository helper, naming `functional_owner_department` as the canonical scoping column for capability-scoped consumers (item 3) and `requested_by_department` for the existing request-scoped consumer (`overrides/pending`, behavior-preserving). Lands *before* item 3 so the new list endpoint consumes it rather than adding a third copy. `exceptions/pending` is untouched by this item.
...
## 7. References
- Pending-list endpoints and their scoping (`overrides/pending`'s department-membership predicate is the pattern items 2 and 3 extend; `exceptions/pending`'s named-reviewer-assignment predicate is a distinct, untouched mechanism — see §A's corrected note): `server/src/modules/capabilityRegistry/routes.ts`, `CapabilityOverrideController.listPending`, `CapabilityOverrideExceptionController.listPending`
