...
- `typeset.css` is vendored at [app/typeset.css](app/typeset.css) (downloaded from `https://ui.shadcn.com/typeset.css`) and imported in [app/globals.css](app/globals.css) as the file's literal first statement. Two build-time corrections were needed against this project's actual toolchain (Tailwind v3 + Turbopack, `next dev`'s default bundler in this Next.js 16 project — shadcn's `typeset` docs are written for Tailwind v4):
  1. **Import position**: moved to the file's first line, not directly after the `@tailwind` directives as initially placed (that built under webpack but broke under Turbopack — the CSS spec's "`@import` must precede all other rules" requirement is enforced at the compiled-CSS level once `@tailwind base/components/utilities` expand into plain rules, not source order, and Turbopack throws on it where webpack's `postcss-import` handling had tolerated it).
  2. **`@layer components` unwrapped**: `typeset.css`'s two `@layer components { ... }` wrappers are stripped (contents unwrapped to plain top-level rules) — Turbopack runs Tailwind's PostCSS plugin against `typeset.css` as its own independent file and rejects `@layer components` with no matching `@tailwind components` directive in that same file, regardless of import order relative to `globals.css`. Tailwind v3's own generated output is unlayered plain CSS anyway, so this is a correct adaptation for this project, not just a workaround — but it means `typeset.css` is **not** byte-identical to the upstream download; re-vendoring requires re-applying both corrections (documented in `.agents/skills/adpa-typeset-markdown-styling/SKILL.md`).
...
2. ~~Import it in `app/globals.css`.~~ **Done, as the file's first statement** (not after the Tailwind directives as originally placed — see §3, Turbopack build fix).
