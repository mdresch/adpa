  ...
  openai: {
    ...
    envKeys: {
      'OPENAI_API_KEY': {
        ...
        required: true,
        validate: (value) => value.startsWith('sk-') || 'OpenAI API key must start with "sk-"'
      },
      'OPENAI_ORGANIZATION': {
  ...
  google: {
    ...
    envKeys: {
      'GOOGLE_AI_API_KEY': {
        ...
        required: true,
        validate: (value) => value.length > 10 || 'Google AI API key must be valid'
      }
    },
  ...
  anthropic: {
    ...
    envKeys: {
      'ANTHROPIC_API_KEY': {
        ...
        required: true,
        validate: (value) => value.startsWith('sk-ant-') || 'Anthropic API key must start with "sk-ant-"'
      }
    },
...
class AIProviderSelector {
   ...
   */
  async updateEnvironmentFiles(config) {
   ...
   */
  async removeFromEnvironmentFiles(keysToRemove) {
    const primaryEnvFile = ENV_FILES[0]; // .env.local
...
module.exports = AIProviderSelector;