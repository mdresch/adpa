...
-- KISS Architecture: System Prompt = Template, User = Context
...
INSERT INTO templates (
  ...
  -- SYSTEM PROMPT (Pure methodology - no variables)
  '## System Prompt: PMBOK Guide 7th Edition Project Management Plan Generator
...
- Output must be complete, structured Markdown document',
  ...
  -- CONTENT (Empty blocks - using system prompt only)
  '{"blocks":[]}',
  
  -- VARIABLES (7 user context fields as JSON array)
  '[
    {
      "name": "projectName",
      "type": "text",
  ...
  -- METADATA
  true,  -- is_public
  0.75,  -- quality_threshold (75% - high quality for formal PMPs)
  1,     -- prompt_version
  'draft', -- development_status (start as draft)
  (SELECT id FROM users WHERE email = 'admin@adpa.com' LIMIT 1), -- created_by (admin user)
  NOW(), -- created_at
  NOW()  -- updated_at
)
