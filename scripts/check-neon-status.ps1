...
function Get-SchemaInfo {
    Write-Host "🏗️ Getting schema information..." -ForegroundColor Yellow
    
    $env:PGPASSWORD = $NEON_PASSWORD
    $connectionString = "host=$NEON_HOST port=$NEON_PORT dbname=$NEON_DB user=$NEON_USER sslmode=$NEON_SSL"
    
    try {
        $tableCount = psql $connectionString -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_type = 'BASE TABLE';" 2>$null
        if ($tableCount) {
            $tableCount = $tableCount.Trim()
            Write-Host "   📋 Total tables: $tableCount" -ForegroundColor Gray
        }
        
        $extensionCount = psql $connectionString -t -c "SELECT COUNT(*) FROM pg_extension;" 2>$null
        if ($extensionCount) {
            $extensionCount = $extensionCount.Trim()
            Write-Host "   🔌 Extensions: $extensionCount" -ForegroundColor Gray
        }
    } catch {
        Write-Host "   ❌ Error getting schema info: $($_.Exception.Message)" -ForegroundColor Red
    } finally {
        Remove-Item Env:PGPASSWORD -ErrorAction SilentlyContinue
    }
}
