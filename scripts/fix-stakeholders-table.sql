-- Fix stakeholders table schema
-- This script adds missing columns to the stakeholders table

-- First, check if the table exists
DO $$ 
BEGIN
    -- Drop the old stakeholders table if it exists (backup any data first!)
    -- Note: This will delete existing stakeholder data
    ...
    CREATE INDEX idx_stakeholders_engagement_approach ON stakeholders(engagement_approach);
    CREATE INDEX idx_stakeholders_stakeholder_type ON stakeholders(stakeholder_type);
    
    RAISE NOTICE 'Stakeholders table has been recreated successfully';
END $$;
