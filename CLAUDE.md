...
Existing packets (manifest `id`s): `rag`, `doc-gen`, `test-teardown-guard`, `codacy`, `infrastructure`, `inline-extraction`, `document-generation-resiliency`, `knowledge-graph`, `compliance`, `ip-governance`, `document-dependency-graph`, `cascading-regeneration`, `api-worker-split`, `template-lifecycle`, `projects`, `federated-capability-ownership`.
