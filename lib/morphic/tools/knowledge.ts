    ...
    execute: async function* ({ sqlQuery }) {
        ...
        try {
            const resultPayload = await db.transaction(async (tx) => {
                ...
                // sql.raw() is used here because the SQL string is dynamically generated by the LLM
                // and cannot be expressed as Drizzle's parameterized query builder (which requires
                // statically known table/column names). The validation above plus SET TRANSACTION READ ONLY
                // ensure that only safe SELECT statements reach this point.
