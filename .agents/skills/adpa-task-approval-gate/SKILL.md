...
| `orchestrator/Adpa.Orchestrator/Controllers/CapabilityController.cs` | Phase 6: `POST api/capability/{moduleId}/{portfolioId}/promote`. Extended (2026-07-13) with six more actions — `override/request`, `override/{requestId}/approve`, `override/{requestId}/deny`, `exceptions/request`, `exceptions/{exceptionId}/reviews/{reviewId}/decide`, `exceptions/{exceptionId}/activate` — all through one shared `Relay` helper, same shape: not `[Authorize]`-decorated (same reasoning as `phase0/approve`/`rtm/apply-amendment` above), relays the caller's bearer token per-request |
