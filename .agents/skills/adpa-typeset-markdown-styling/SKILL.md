...
| Piece | File | Status |
|---|---|---|
| Stylesheet | `app/typeset.css` | Downloaded verbatim from `https://ui.shadcn.com/typeset.css`; do not hand-edit — re-download to update. |
| Import | `app/globals.css` | `@import "./typeset.css";` immediately after the three `@tailwind` directives. |
