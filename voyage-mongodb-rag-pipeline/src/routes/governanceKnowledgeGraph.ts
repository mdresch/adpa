...
import { gkgSync } from '../services/governanceKnowledgeGraphSync';
...
router.post('/bootstrap', async (req, res) => {
  try {
    ...
    const success = await gkgSync.bootstrapGKG();
    ...
    logger.error('GKG bootstrap error:', error);
    res.status(500).json({
      success: false,
...
router.post('/sync-all', async (req, res) => {
  try {
    logger.info('Starting bulk project sync to GKG...');
    
    const result = await gkgSync.syncAllProjects();
...
router.post('/sync-project/:projectId', async (req, res) => {
  try {
    ...
    logger.info(`Syncing project ${projectId} to GKG...`);
    ...
    const result = await gkgSync.syncProjectToGKG(project);
    ...
    logger.error(`Project sync error for ${req.params.projectId}:`, error);
    res.status(500).json({
      success: false,
...
router.get('/status', async (req, res) => {
  try {
    ...
    logger.error('GKG status error:', error);
    res.status(500).json({
      success: false,
...
router.get('/project/:projectId/graph', async (req, res) => {
  try {
    ...
    logger.error(`Project graph error for ${req.params.projectId}:`, error);
    res.status(500).json({
      success: false,
...
router.get('/entity/:entityId/relationships', async (req, res) => {
  try {
    ...
    logger.error(`Entity relationships error for ${req.params.entityId}:`, error);
    res.status(500).json({
      success: false,
